...
计数排序基于一个假设，待排序数列的所有数均为整数，且出现在一个很小的范围内，例如 0 到 100 之间
...
[^_^]: 这个基础版的计数排序被注释掉了
    ...
    计数排序的基本思想：（基础版）
    1. 统计每一种可能的值 `i` 在序列中出现的次数
    2. 从小到大枚举所有 `i` ，按照出现次数输出对应数量的 `i`

    代码实现：
    ```cpp
    void CountSort(vector<int> &a) { // 计数排序的简单实现
        int n = a.size();
        // 找出待排序数组的最大值 Max
        int Max = a[0];
        for (int i = 1; i < n; i++)
        ...
        // 维护最终有序序列
        vector<int> b(n,0);
        for (int i = 0, j = 0; i <= Max; i++)    // 枚举每一种值 i ，指针 j 用来枚举填充答案数组中的位置
...
1. 找出待排序数组中的最大值 `MaxValue`
    - 假设最小值为 `0`

2. 统计数组中每个值为 `i` 元素的出现次数，存入 `cnt[i]`

3. 对 `cnt` 数组求前缀和，更新 `cnt` 数组
    - 求前缀和，即：从 `i = 1` 开始（ `cnt[0]` 保持不变），执行 `cnt[i] = cnt[i] + cnt[i - 1]`
    - 更新后的 `cnt[i]` 的含义是：小于等于 `i` 的数字个数。即，目标数组的第 `cnt[i] - 1` 位上是数组中最后一个 `i`

4. 从右向左填充目标数组：将每个值 `i` 放在新数组的第 `cnt[i] - 1` 位，即，作为第 `cnt[i]` 个元素，并执行 `cnt[i]--`
    - 等价于，将值 `i` 填充在新数组的第 `--cnt[i]` 位
...
void CountSort(vector<int>& nums, vector<int>& ans) {
    ...
    // 统计每一种值出现的次数
    vector<int> cnt(MaxValue + 1, 0);   // 使用 MaxValue + 1 作为 cnt 的维度
    for (int i = 0; i < n; ++i)
        ++cnt[nums[i]];
    ...
    for (int i = n - 1; i >= 0; i--)    // 逆序填充，确保排序的稳定性
        ans[--cnt[nums[i]]] = nums[i];  // 注意：nums[i] 的填充位置为 --cnt[nums[i]]
}
```

> 这里统计的数字范围为 `[0, MaxValue]` ，事实上，也可以找出数组元素的最小值 `MinValue` ，仅统计 `[MinValue, MaxValue]` 区间内的数字。即，用 `cnt[0]` 记录值 `MinValue` 的出现次数，用 `cnt[MaxValue - MinValue]` 记录值 `MaxValue` 的出现次数，并且，对每个值 `i` ，执行 `ans[--cnt[i - MinValue]] = i` 以填充目标序列
